...
void ObjModel::loadObjModel(string path)
{
...
Mesh ObjModel::processMesh(aiMesh* mesh, const aiScene* scene)
{
	...
	for (GLuint i = 0; i < mesh->mNumVertices; i++)
	{
		...
		if (mesh->mTextureCoords[0]) // Does the mesh contain texture coordinates?
		{
			...
			// use ObjModels where a vertex can have multiple texture coordinates so we always take the first set (0).
...
vector<Texture> ObjModel::loadMaterialTextures(aiMaterial* mat, aiTextureType type, string typeName)
{
	...
	for (GLuint i = 0; i < mat->GetTextureCount(type); i++)
	{
		...
		{   // If texture hasn't been loaded already, load it
			...
			this->textures_loaded.push_back(texture);  // Store it as texture loaded for entire ObjModel, to ensure we won't unnecesery load duplicate textures.
		}
	}
	return textures;
}
