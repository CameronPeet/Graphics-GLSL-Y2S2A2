...
ShaderVariables ShaderLoader::CreateShaderProgram(char* vertexShaderFilename, char* fragmentShaderFilename, char* geometryShaderFilename)
{
	//read the shader files and save the code
	std::string vertex_shader_code = ReadShader(vertexShaderFilename);
	std::string fragment_shader_code = ReadShader(fragmentShaderFilename);
	...
	if (link_result == GL_FALSE)
	{
		...
		shaderVariables.OK = false;
		return shaderVariables;
	}

	shaderVariables.program = program;
	shaderVariables.vertexShader = vertex_shader;
	shaderVariables.fragmentShader = fragment_shader;
	shaderVariables.geometryShader = geometry_shader;
	shaderVariables.OK = true;

	return shaderVariables;
}


GLuint ShaderLoader::CreateComputeShaderProgram(char* computeShaderFilename)
{
